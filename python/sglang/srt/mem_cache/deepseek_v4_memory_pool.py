...
class DeepSeekV4TokenToKVPool(BaseSWAKVPool):
    ...
    def load_cpu_copy(self, kv_cache_cpu, indices, mamba_indices=None):
        ...
        c4_indexer_cpu = kv_cache_cpu.get("c4_indexer")
        if c4_indexer_cpu is not None:
            self.c4_indexer_kv_pool.load_cpu_copy(c4_indexer_cpu, c4_indices)
        c128_cpu = kv_cache_cpu.get("c128")
        if c128_cpu is not None:
            self.c128_kv_pool.load_cpu_copy(c128_cpu, c128_indices)
